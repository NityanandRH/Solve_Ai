...
import agents
import prompts
from User import UserOut, TokenData, extract_sso_identity
...
# Azure OpenAI client — uses your org's Azure OpenAI deployment
...
async def get_current_user(request: Request) -> TokenData:
    """
    Extracts the authenticated user from App Service Easy Auth headers.
    Auto-provisions the user in the DB on first login.
    In local dev (LOCAL_DEV=true) uses a mock identity from .env.
    """
    identity = extract_sso_identity(dict(request.headers))
    if not identity:
        raise HTTPException(
            status_code=401,
            detail="Not authenticated. Ensure Azure App Service Easy Auth is enabled."
        )

    # Auto-provision / sync user on every request
    db_user = await utils.get_or_create_sso_user(
        aad_oid=identity["oid"],
        upn=identity["upn"],
    ...
    return TokenData(
        ...
        user_id=db_user["id"],
        role=db_user["role"],
        email=db_user.get("email") or db_user["username"],
...
# ── Auth Routes ───────────────────────────────────────────────────────────────
# No login endpoint needed — Azure App Service Easy Auth handles the AAD
# redirect/token exchange before requests reach FastAPI.
# Users land directly on the app after org SSO.

@app.get("/api/auth/me", response_model=UserOut)
async def get_me(current_user: TokenData = Depends(get_current_user)):
    """
    Returns the currently authenticated user.
    Called by the frontend on load to get identity and role.
    Also triggers auto-provisioning of new users on first login.
    """
    return UserOut(
...
async def logout():
    """
    Redirect to App Service logout endpoint which clears the AAD session.
    Frontend should navigate to /.auth/logout for full sign-out.
    """
    from fastapi.responses import RedirectResponse
    return RedirectResponse(url="/.auth/logout?post_logout_redirect_uri=/")
